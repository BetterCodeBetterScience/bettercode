...
def ricker_model(N: float, r: float, sigma: float = 0.3, phi: float = 10) -> Tuple[float, float]:
    """
...
def compute_credible_intervals(
    ...
    for i, name in enumerate(param_names):
        ...
        if verbose:
            ...
            print(f"  Posterior mean: {samples_np[:, i].mean():.3f} ± {samples_np[:, i].std():.3f}")
...
def run_ricker_sbi(
    ...
    **kwargs : dict
    ...
    def simulator(params):
        return ricker_simulator(params, n_time_steps=n_time_steps, starting_n=starting_n)
